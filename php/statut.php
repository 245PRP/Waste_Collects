...
// $non_traites = $cnx->query("SELECT * FROM ramassage WHERE statut='en attente' ORDER BY date_signal DESC")->fetchAll(PDO::FETCH_ASSOC);
...
$stmt = $cnx->prepare("SELECT * FROM ramassage, point_collecte WHERE ramassage.id_pt=point_collecte.id_pt AND id_user = :id AND statut = 'en attente'");
    $stmt->execute(['id' => $id]);
    $non_traites = $stmt->fetchAll(PDO::FETCH_ASSOC);

// ✅ Récupérer les signalements traités
 $stmt2 = $cnx->prepare("SELECT * FROM ramassage, point_collecte WHERE ramassage.id_pt=point_collecte.id_pt AND id_user = :id AND statut = 'traitee'");
    $stmt2->execute(['id' => $id]);
    $traitees = $stmt2->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body>
  ...
  <div class="main">
    ...
    <h2>Suivi des tournéess</h2>

    <!-- Onglets -->
    <div class="tabs">
      <div class="tab-btn active" onclick="showTab('non')">Tournées non traités</div>
      <div class="tab-btn" onclick="showTab('oui')">Tournées traitées</div>
    </div>

    <!-- Signalements non traités -->
    <div id="tab-non">
      <?php if (!empty($non_traites)): ?>
        <?php foreach($non_traites as $nons): ?>
          <div class="notif-item">
            <strong><?= htmlspecialchars($nons['nom_pt']) ?></strong><br>
            <span class="notif-date"><?= date("d/m/Y H:i", strtotime($nons['date_tour'])) ?></span><br>
            
          </div>
        <?php endforeach; ?>
      <?php else: ?>
        <p>Aucune tournée non traité.</p>
      <?php endif; ?>
    </div>
      ...
      <?php if (!empty($traitees)): ?>
        <?php foreach($traitees as $s): ?>
          <div class="notif-item">
            ...
            <span class="notif-date"><?= date("d/m/Y H:i", strtotime($s['date_tour'])) ?></span><br>
            ✅ <em>Traité</em>
          </div>
        <?php endforeach; ?>
      <?php else: ?>
